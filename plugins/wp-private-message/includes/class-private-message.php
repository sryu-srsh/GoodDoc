...
class WP_Private_Message_Message {
	...
	public static function process_send_message() {
		...
		$recipient = !empty($_POST['recipient']) ? sanitize_text_field($_POST['recipient']) : '';
	...
	public static function insert_message($user_id, $recipient) {
		$message_args = array(
            'post_title' => isset($_POST['subject']) ? sanitize_text_field($_POST['subject']) : '',
            'post_type' => 'private_message',
            'post_content' => isset($_POST['message']) ? sanitize_text_field($_POST['message']) : '',
            'post_status' => 'publish',
            'post_author' => $user_id,
        );
	...
	public static function process_reply_message() {
		...
		$parent = !empty($_POST['parent']) ? sanitize_text_field($_POST['parent']) : '';
		...
		if ( $user_id != $sender && $user_id != $recipient ) {
			$return = array( 'status' => false, 'msg' => esc_html__('You have not permission to reply this message.', 'wp-private-message') );
		   	echo wp_json_encode($return);
		   	exit;
		}
	...
	public static function process_choose_message() {
    	...
    	$recipient = get_post_meta($message_id, '_recipient', true);
		...
		if ( $user_id == $sender || $user_id == $recipient ) {
			...
			$return = array( 'status' => false, 'msg' => esc_html__('You have not permission to view this message', 'wp-private-message') );
		   	echo wp_json_encode($return);
		   	exit;
		}
	}
	...
	public static function process_search_message() {
		...
		$search_read = isset($_POST['search_read']) ? sanitize_text_field($_POST['search_read']) : '';
	...
	public static function process_delete_message() {
		...
		if ( $user_id != $sender && $user_id != $recipient ) {
			$return = array( 'status' => false, 'msg' => esc_html__('You have not permission to delete this message.', 'wp-private-message') );
		   	echo wp_json_encode($return);
		   	exit;
		}
